...
class RuleEngine:
    ...
    def _boost_confidence_for_completeness(
        ...
        """Add up to +0.13 when the vision result is thorough and internally consistent."""
        boost = 0.0

        # Property type clearly identified
        if prop_type_raw not in ("unknown", ""):
            boost += 0.03

        # All critical boolean flags explicitly answered
        critical = [
        ...
        # Model wrote a meaningful notes field
        notes = str(vr.get("notes", "")).strip()
        if len(notes) > 40:
            boost += 0.02

        # Internal consistency bonus: key field pairs that should agree do agree
        has_structure = vr.get("has_structure", True)
        vacancy_signs = vr.get("vacancy_signs", False)
        prop_type = str(vr.get("property_type", "")).lower()

        # Consistent: no structure + no vacancy signs (correct — vacancy requires a building)
        if not has_structure and not vacancy_signs:
            boost += 0.02
        # Consistent: residential/commercial/mobile_home all confirmed has_structure=true
        if prop_type in ("residential", "commercial", "mobile_home", "industrial") and has_structure:
            boost += 0.01

        # Verification pass ran (notes contain "| Verified:")
        if "| Verified:" in notes:
            boost += 0.02

        return round(min(base + boost, 0.97), 2)
